...
base_path = r'local/images'
capture_path = r'capture'
reference_path = r'reference'
delta_path = r'delta'
log_path = r'detection'
...
def find_unprocessed_captures(mdy,basedir=base_path):
    '''
        mdy = <Month> <Day> <Year>
        e.g. June 11 2017
    '''
    ...
    with task_status('Finding unprocessed captures'):
        for fname in os.listdir(os.path.join(basedir,capture_path,mdy)):
            if not os.path.exists(os.path.join(basedir,delta_path,mdy,fname)):
                try:
                    ref_name = reference_fits_name(fname)
                except:
                    print('Cannot determine reference file for file:',fname)
                else:
                    if not os.path.exists(os.path.join(basedir,reference_path,ref_name)):
...
#Image Processing    
def process_ccd(data):
    return cosmic(ccdproc.median_filter(data,2))[0]
...
def save_source_log(mdy,cap_fname,sources,ref=False,basedir=base_path):
    os.makedirs(os.path.join(basedir,log_path,mdy),exist_ok=True)
    prefix,num,obj_name = parse_fits_name(cap_fname)
    if ref:
        log_fpath = os.path.join(basedir,log_path,mdy,prefix + '.'  + num + '.' + obj_name + '.ref.log')
    else:
        log_fpath = os.path.join(basedir,log_path,mdy,prefix + '.'  + num + '.' +  obj_name + '.log')
    sources.write(log_fpath,format='ascii.ecsv',overwrite=True)
    #with open(log_fpath,'w') as f:
    #    f.write(str(sources))
    
def save_source_plot(mdy,cap_fname,sources,im_data,ref=False,basedir=base_path):
    os.makedirs(os.path.join(basedir,log_path,mdy),exist_ok=True)
    ...
    prefix,num,obj_name = parse_fits_name(cap_fname)
    if ref:
        plot_fpath = os.path.join(basedir,log_path,mdy,prefix + '.' + num + '.' + obj_name + '.ref.png')
    else:
        plot_fpath = os.path.join(basedir,log_path,mdy,prefix + '.' +  num + '.' + obj_name + '.png')
    plt.savefig(plot_fpath)
    plt.close()
    

def copy_to_reference(y=None,m=None,d=None,mdy=None,basedir=base_path):
    if y == None:
        if mdy == None:
            mdy = today_mdy()
    else:
    ...
    cap_path = os.path.join(basedir,capture_path,mdy)
    
    with task_status('Copying files to reference:',mdy):
        for fname in sorted(os.listdir(cap_path)):
            capture_fpath = os.path.join(basedir,capture_path,mdy,fname)
            ref_name = reference_fits_name(fname)
            ref_fpath = os.path.join(basedir,reference_path,ref_name)
            with task_status('Copying',fname):
                copyfile(capture_fpath,ref_fpath)
        
        
def process_images(y=None,m=None,d=None,mdy=None,basedir='.',only_new=True):
    if y == None:
        if mdy == None:
            mdy = today_mdy()
    else:
    ...
    if not valid_mdy(mdy):
        raise Exception('Invalid date folder name:',mdy)
    to_process = find_unprocessed_captures(mdy,basedir=basedir)
    for fname in to_process:
        capture_fpath = os.path.join(basedir,capture_path,mdy,fname)
        ref_name = reference_fits_name(fname)
        ref_fpath = os.path.join(basedir,reference_path,ref_name)
        delta_fpath = os.path.realpath(os.path.join(basedir,delta_path,mdy,fname))
        
        with task_status('Loading captured file:',fname):
            imcap = process_ccd(load_fits_data(capture_fpath))
        with task_status('Loading reference file:',ref_name):
            imref = process_ccd(load_fits_data(ref_fpath))
        ...
        with task_status('Saving delta'):
            os.makedirs(os.path.join(basedir,delta_path,mdy),exist_ok=True)
        ...
        with task_status('Saving source detection results'):
            save_source_log(mdy,fname,sources,basedir=basedir)
            save_source_log(mdy,fname,refsources,ref=True,basedir=basedir)
            save_source_plot(mdy,fname,sources,imreg,basedir=basedir)
            save_source_plot(mdy,fname,refsources,imref,ref=True,basedir=basedir)
